...
const KepokmasPriceTable = () => {
  ...
  const filteredSurveys = priceSurveys.filter(survey => {
    ...
    return matchesSearch && matchesMarket && matchesCommodity;
  });

  const loadData = async () => {
    try {
      // Load price surveys for current month
      ...
      const [surveysResult, commoditiesResult, marketsResult] = await Promise.all([
        supabase
          .from('price_surveys')
          .select('*')
          .gte('survey_date', `${currentMonth}-01`)
    ...
    return (
      <Card>
        <CardContent className="p-6">
          <div className="text-center">Memuat data harga komoditas...</div>
        </CardContent>
      </Card>
    );
  }
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <div>
          <h2 className="text-2xl font-bold">Harga Komoditas Kepokmas</h2>
          <p className="text-muted-foreground">
            Menampilkan {filteredSurveys.length} dari {priceSurveys.length} data harga bulan ini
          </p>
        </div>
        <Button>
      ...
      <Card>
        <CardHeader>
          <CardTitle>Filter Data Harga</CardTitle>
        </CardHeader>
        ...
        <CardContent className="p-6">
          ...
          {filteredSurveys.length === 0 && (
            <div className="text-center py-8 text-muted-foreground">
              Tidak ada data harga yang ditemukan untuk filter ini
            </div>
          )}
        </CardContent>
      </Card>
...
export default KepokmasPriceTable;