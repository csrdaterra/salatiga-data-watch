...
const agenRealisasiBbmSchema = z.object({
...
type AgenRealisasiBbmFormData = z.infer<typeof agenRealisasiBbmSchema>;

interface AgenRealisasiBbm {
  id: string;
  nama_usaha: string;
  nomor_spbu: string;
  kecamatan: string;
...
export function AgenRealisasiBbmTable() {
  const [data, setData] = useState<AgenRealisasiBbm[]>([]);
  const [loading, setLoading] = useState(true);
  const [isDialogOpen, setIsDialogOpen] = useState(false);
  const [editingItem, setEditingItem] = useState<AgenRealisasiBbm | null>(null);
  const { toast } = useToast();

  const form = useForm<AgenRealisasiBbmFormData>({
    resolver: zodResolver(agenRealisasiBbmSchema),
    defaultValues: {
      nama_usaha: "",
      nomor_spbu: "",
      kecamatan: "",
  ...
  const fetchData = async () => {
    try {
      ...
      console.error("Error fetching Agen Realisasi BBM data:", error);
      toast({
        title: "Error",
        description: "Gagal memuat data Realisasi BBM",
        variant: "destructive",
      });
    } finally {
      setLoading(false);
  ...
  const onSubmit = async (formData: AgenRealisasiBbmFormData) => {
    try {
      const submitData = {
        ...
        target_bulanan: formData.target_bulanan ? parseFloat(formData.target_bulanan) : null,
        realisasi_bulanan: formData.realisasi_bulanan ? parseFloat(formData.realisasi_bulanan) : null,
      ...
      if (editingItem) {
        ...
        toast({
          title: "Berhasil",
          description: "Data Realisasi BBM berhasil diperbarui",
        });
      } else {
        const { error } = await supabase
          .from("agen_realisasi_bbm")
        ...
        toast({
          title: "Berhasil",
          description: "Data Realisasi BBM berhasil ditambahkan",
        });
      }
      ...
      toast({
        title: "Error",
        description: "Gagal menyimpan data Realisasi BBM",
        variant: "destructive",
      });
    }
  };

  const handleEdit = (item: AgenRealisasiBbm) => {
    setEditingItem(item);
    form.reset({
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      ...
      toast({
        title: "Berhasil",
        description: "Data Realisasi BBM berhasil dihapus",
      });
      fetchData();
    } catch (error) {
      ...
      toast({
        title: "Error",
        description: "Gagal menghapus data Realisasi BBM",
        variant: "destructive",
      });
    }
  };
  ...
  return (
    <div className="space-y-4">
      <div className="flex justify-between items-center">
        <h3 className="text-lg font-semibold">Data Realisasi BBM</h3>
        <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
          <DialogTrigger asChild>
            <Button onClick={handleAddNew}>
              <Plus className="h-4 w-4 mr-2" />
              Tambah Realisasi BBM
            </Button>
          </DialogTrigger>
          <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
            <DialogHeader>
              <DialogTitle>
                {editingItem ? "Edit Realisasi BBM" : "Tambah Realisasi BBM"}
              </DialogTitle>
            </DialogHeader>
            <Form {...form}>
              <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        <FormLabel>Target Bulanan (Liter)</FormLabel>
                        <FormControl>
                  ...
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        <FormLabel>Realisasi Bulanan (Liter)</FormLabel>
                        <FormControl>
                          <Input {...field} type="number" />
                        </FormControl>
                        <FormMessage />
        ...
        <Table>
          <TableHeader>
            <TableRow>
              ...
              <TableHead>Realisasi (L)</TableHead>
