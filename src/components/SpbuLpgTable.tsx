...
type SpbuLpgFormData = z.infer<typeof spbuLpgSchema>;
...
export function SpbuLpgTable() {
  const [data, setData] = useState<SpbuLpg[]>([]);
  const [loading, setLoading] = useState(true);
  const [isDialogOpen, setIsDialogOpen] = useState(false);
  const [editingItem, setEditingItem] = useState<SpbuLpg | null>(null);
  const { toast } = useToast();

  const form = useForm<SpbuLpgFormData>({
    resolver: zodResolver(spbuLpgSchema),
    defaultValues: {
      nama_usaha: "",
      nomor_spbu: "",
      kecamatan: "",
  ...
  const fetchData = async () => {
    try {
      const { data: spbuData, error } = await supabase
        .from("spbu_lpg")
        .select("*")
        .order("nama_usaha");
      ...
      console.error("Error fetching SPBU LPG data:", error);
      toast({
        title: "Error",
        description: "Gagal memuat data SPBU LPG",
        variant: "destructive",
      });
    } finally {
      setLoading(false);
  ...
  const onSubmit = async (formData: SpbuLpgFormData) => {
    try {
      const submitData = {
      ...
      if (editingItem) {
        ...
        toast({
          title: "Berhasil",
          description: "Data SPBU LPG berhasil diperbarui",
        });
      } else {
        const { error } = await supabase
          .from("spbu_lpg")
        ...
        toast({
          title: "Berhasil",
          description: "Data SPBU LPG berhasil ditambahkan",
        });
      }
      ...
      toast({
        title: "Error",
        description: "Gagal menyimpan data SPBU LPG",
        variant: "destructive",
      });
    }
  };

  const handleEdit = (item: SpbuLpg) => {
    setEditingItem(item);
    form.reset({
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      ...
      toast({
        title: "Berhasil",
        description: "Data SPBU LPG berhasil dihapus",
      });
      fetchData();
    } catch (error) {
      ...
      toast({
        title: "Error",
        description: "Gagal menghapus data SPBU LPG",
        variant: "destructive",
      });
    }
  };
  ...
  return (
    <div className="space-y-4">
      <div className="flex justify-between items-center">
        <h3 className="text-lg font-semibold">Data SPBU LPG</h3>
        <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
          ...
          <DialogContent className="max-w-md">
            <DialogHeader>
              <DialogTitle>
                {editingItem ? "Edit SPBU LPG" : "Tambah SPBU LPG"}
              </DialogTitle>
            </DialogHeader>
            <Form {...form}>
