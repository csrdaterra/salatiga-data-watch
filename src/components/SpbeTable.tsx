...
type SpbeFormData = z.infer<typeof spbeSchema>;
...
export function SpbeTable() {
  const [data, setData] = useState<Spbe[]>([]);
  const [loading, setLoading] = useState(true);
  const [isDialogOpen, setIsDialogOpen] = useState(false);
  const [editingItem, setEditingItem] = useState<Spbe | null>(null);
  const { toast } = useToast();

  const form = useForm<SpbeFormData>({
    resolver: zodResolver(spbeSchema),
    defaultValues: {
      nama_usaha: "",
      nomor_spbu: "",
      kecamatan: "",
  ...
  const fetchData = async () => {
    try {
      const { data: spbeData, error } = await supabase
        .from("spbe")
        .select("*")
        .order("nama_usaha");
      ...
      console.error("Error fetching SPBE data:", error);
      toast({
        title: "Error",
        description: "Gagal memuat data SPBE",
        variant: "destructive",
      });
    } finally {
      setLoading(false);
  ...
  const onSubmit = async (formData: SpbeFormData) => {
    try {
      const submitData = {
      ...
      if (editingItem) {
        ...
        toast({
          title: "Berhasil",
          description: "Data SPBE berhasil diperbarui",
        });
      } else {
        const { error } = await supabase
          .from("spbe")
        ...
        toast({
          title: "Berhasil",
          description: "Data SPBE berhasil ditambahkan",
        });
      }
      ...
      toast({
        title: "Error",
        description: "Gagal menyimpan data SPBE",
        variant: "destructive",
      });
    }
  };

  const handleEdit = (item: Spbe) => {
    setEditingItem(item);
    form.reset({
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      ...
      toast({
        title: "Berhasil",
        description: "Data SPBE berhasil dihapus",
      });
      fetchData();
    } catch (error) {
      ...
      toast({
        title: "Error",
        description: "Gagal menghapus data SPBE",
        variant: "destructive",
      });
    }
  };
  ...
  return (
    <div className="space-y-4">
      <div className="flex justify-between items-center">
        <h3 className="text-lg font-semibold">Data SPBE</h3>
        <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
          ...
          <DialogContent className="max-w-md">
            <DialogHeader>
              <DialogTitle>
                {editingItem ? "Edit SPBE" : "Tambah SPBE"}
              </DialogTitle>
            </DialogHeader>
            <Form {...form}>
